...
Public Class Rpa01 : Inherits Rpa00.RpaBase(Of Rpa01)
    ...
    Private ReadOnly Property WorkSMCsvFileName As String
        Get
            Return $"{Me.WorkDirectory}\{Path.GetFileName(Me.MasterCsvFileName)}"
        End Get
    End Property
    ...
    Private Function Main(ByRef dat As Object) As Integer
        ...
        File.Copy(Me.MasterCsvFileName, Me.WorkSMCsvFileName, True)
        '-----------------------------------------------------------------------------------------'
    ...
    Private Function Check(ByRef dat As Object) As Boolean
        ...
        ' 送付明細ファイルチェック
        '-----------------------------------------------------------------------------------------'
        If Not File.Exists(Me.MasterCsvFileName) Then
            Call dat.System.RpaWriteLine($"'MasterCsvFileName' が指定されていません")
            Return False
        End If
        ...
        If String.IsNullOrEmpty(Me.AttacheCase) Then
            Call dat.System.RpaWriteLine($"'AttacheCase' が指定されていません")
            Return False
        End If
        ...
        ' 口座振替スケジュールファイルのチェック(2021/02/08追加)
        '-----------------------------------------------------------------------------------------'
